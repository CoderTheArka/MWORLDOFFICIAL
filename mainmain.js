...
}else if(localStorage.getItem('information') == undefined){
  window.location = 'index.html'
}
...
firebase.database().ref('GS/').child(localStorage.getItem('dbv')).update({
ONLINE:'YES'
});
}
setTimeout(function(){
...
function checksomthing(){
  ...
  var div_start = "<div id="+NAME+" onmousemove='getthe(this.id)' class='d-flex align-items-start'>";
  ...
  var name2 = NAME;
  var mem_start1 = '<div  id="change" color: "black;" class="small"><span class="fa fa-circle fa fa-link dfdf"></span> USER OF MWORLD</div>'
  var div_1_end = '</div>';
  var div_end = '</div>';
  var a_end = '</a>';
  var hr = '<hr>';
...
function getthe(getr){

  var ref = firebase.database().ref('GS/').child(getr);
  ref.once('value',function(get){
  var data = get.val();
  if (data.ONLINE == 'YES'){
  document.getElementById('change').innerHTML = '<span class="fa fa-circle fa fa-link dfdf"></span> ONLINE :)'
  }else if(data.ONLINE == 'NO'){
 document.getElementById('change').innerHTML = '<i class="fa fa-circle" style="color:red"></i> OFFLINE :/'
  }
  });
}
...
function get12(dd){
  console.log('HappyHaaa.')
  console.log(dd);
  console.log(localStorage.getItem('dbv'));
  var ref7 = firebase.database().ref('REMOVEM/'+dd+'/').child(localStorage.getItem('dbv'));
  ref7.once('value',function(get_fo){
  var data69=  get_fo.val();
  console.log('Hello Word 69')
  localStorage.setItem('checksend' ,data69.R);
  localStorage.setItem('addmmm',data69.R);
  });
}
function get1212(dd,gg){
  console.log('HappyHaaa.')
  console.log(dd);
  console.log(localStorage.getItem('dbv'));
  var ref7 = firebase.database().ref('REMOVEM/'+dd+'/').child(gg);
  ref7.once('value',function(get_fo){
  var data69=  get_fo.val();
  console.log('Hello Word 69')
  localStorage.setItem('addmmm',data69.R);
...
{
  localStorage.removeItem('checksend');
   document.getElementById('normal_send').style.display = 'none'
  get12(dd);
...
document.getElementById('ssdsd').style.display = 'block';
  ...
  setTimeout(function(){
  if(localStorage.getItem('checksend') == 'YES'){
    document.getElementById('normal_send').style.display = 'none';
    document.getElementById('elligable').style.display = 'block';

  }else{
    document.getElementById('normal_send').style.display = 'block';
    document.getElementById('elligable').style.display = 'none';
    s();
  }
},2000)
...
console.log(document.getElementById('ssda').innerHTML);
...
console.log(document.getElementById('ssda').innerHTML);
 ...
 var ref1 = firebase.database().ref('REMOVEM/'+document.getElementById('ssda').innerHTML +'/').child(get_member);

 ref1.once('value',function(lj){
var data1 = lj.val();
console.log(data);
if(data1.R == 'YES'){
  document.getElementById('color'+get_member).style.backgroundColor = 'yellow';
  document.getElementById('color'+get_member).style.color = 'blue';
  document.getElementById('text'+get_member).innerHTML = 'THIS PERSON WAS REMOVED FROM THIS GROUP BY ADMINS';
  document.getElementById('text'+get_member).style.color = 'green';
}else{
  return false
}
});
...
{
  localStorage.removeItem('VerifyIDK')
  ...
  setTimeout(function(){
    ...
    if(localStorage.getItem('VerifyIDK') == null){
      ...
      get1212(localStorage.getItem('NAMEOg'),get_nm);

     setTimeout(function(){
 if(localStorage.getItem('addmmm') == 'YES'){

      alert('THIS MEMBER WAS REMOVED  AND WILL AGAIN JOIN THE MEMBER');
       ...
       var data = df.val();
        ...
        setTimeout(function(){
          if(localStorage.getItem('ewjh') == null){
            firebase.database().ref('REMOVEM/'+localStorage.getItem('NAMEOg')+'/').child(get_nm).remove();
...
function makeadmin(){
  if(document.getElementById('addmem').value != ''){
    document.getElementById('addmem').setAttribute('readonly',true)
    alert('First Process Is Starting')
    var ref = firebase.database().ref('Searched Box/').child(document.getElementById('addmem').value);
    ref.once('value',function(fu){
    var data = fu.val();
    var get_email = data.EMAIL;
    var semail = get_email;
    var sdomain = semail.substring(0, semail.lastIndexOf("@"));
    localStorage.setItem('makeadminemail',sdomain);
    });
    alert('Second Step Is Starting in 5sec');
    setTimeout(function(){
      if( localStorage.getItem('makeadminemail') != null){
        var name = localStorage.getItem('makeadminemail');
        var ref = firebase.database().ref('GROUPCHECK/' + localStorage.getItem('NAMEOg') + '/').child(name);
        ref.once('value',function(getu){
        var data = getu.val();
        localStorage.setItem('CheckMake',data.ADMIN)

        });
        alert('We are in the third stage of the process');
        setTimeout(function(){
        var admin = localStorage.getItem('CheckMake');
      if (admin != undefined || admin != null){
        if (admin == 'YES'){
          alert('USER IS ALREADY A ADMIN.')
          localStorage.removeItem('Checkmake');
          localStorage.removeItem('makeadminemail');
          document.getElementById('addmem').removeAttribute('readonly')
        }else{
          alert('YOU HAVE PASSED ALL THE VERIFICATION NOW THE GIVEN USER IS BEING CONVERTED TO ADMIN.')
          firebase.database().ref('GROUPCHECK/' + localStorage.getItem('NAMEOg') + '/').child(localStorage.getItem('makeadminemail')).update({
          ADMIN:'YES',
          GROUP_NAME:localStorage.getItem('NAMEOg')
          });
          alert('USER IS NOW A ADMIN OR IN OP MODE')
          localStorage.removeItem('Checkmake');
          localStorage.removeItem('makeadminemail');
          document.getElementById('addmem').removeAttribute('readonly')
        }
      }else{
        alert('CHECK CAREFULLY THE USER DOSNT EXSISTS IN YOUR SERVER.')
        window.location = 'index.html'
      }

        },3000)
      }else{
        alert('THE USER DOSENT EXSIST IN MWORLD. ')
        window.location = 'index.html'
      }
...
if(document.getElementById('addmem').value != ''){
  document.getElementById('addmem').setAttribute('readonly',true);
  alert('Now we will check if the player really exsists on the server or not.');
  var ref = firebase.database().ref('Searched Box/').child(document.getElementById('addmem').value);
  ref.once('value',function(fu){
  var data = fu.val();
  var get_email = data.EMAIL;
  ...
  alert('Waiting for the verification.');
  setTimeout(function(){
    if( localStorage.getItem('makeadminemail') != null){
      var name = localStorage.getItem('makeadminemail');
      var ref = firebase.database().ref('GROUPCHECK/' + localStorage.getItem('NAMEOg') + '/').child(name);
      ...
      var data = getu.val();
      localStorage.setItem('CheckMake',data.ADMIN);
      localStorage.setItem('GodCheck',data.GOD);

      });
      alert('The User Is Verified now we will check if the user is in your group');
      setTimeout(function(){
      var admin = localStorage.getItem('CheckMake');
    if (admin != null){
      if (admin != 'YES'){
        alert('USER IS NOT A ADMIN OF YOUR GROUP THEREFORE NO CHANGE HAS BEEN DONE.')
        localStorage.removeItem('Checkmake');
        localStorage.removeItem('makeadminemail');
        localStorage.removeItem('GodCheck')
        document.getElementById('addmem').removeAttribute('readonly')

      }else if(localStorage.getItem('GodCheck') != undefined && localStorage.getItem('GodCheck') == 'YES'){
        alert('CANT REMOVE ADMIN POWER OF THAE GOD OF THE GROUP MEANS WHO CREATED THE GROUP.')
        localStorage.removeItem('Checkmake');
        localStorage.removeItem('makeadminemail');
        localStorage.removeItem('GodCheck')
        document.getElementById('addmem').removeAttribute('readonly')
      }else{
        alert('WE HAVE FOUND THAT USER IS A ADMIN AND A MEMBER OF YOUR GROUP NOW WE WILL MAKE THE USER FROM ADMIN TO ONLY A MEMBER')
        firebase.database().ref('GROUPCHECK/' + localStorage.getItem('NAMEOg') + '/').child(localStorage.getItem('makeadminemail')).update({
        ADMIN:'NO',
        GROUP_NAME:localStorage.getItem('NAMEOg')
        });
        alert('THE USER IS ONLY A MEMBER NOT A ADMIN THE PROCESS HAS BEEN ENDED')
        localStorage.removeItem('Checkmake');
        localStorage.removeItem('makeadminemail');
        localStorage.removeItem('GodCheck')
        document.getElementById('addmem').removeAttribute('readonly')
      ...
      alert('THE USER IS NOT IN YOUR GROUP BUT EXSISTS IN MWORLD.')
      window.location = 'index.html'
    }
  ...
  if(document.getElementById('addmem').value != ''){
    ...
    setTimeout(function(){
      ...
      if (admin != null){
        ...
        if(localStorage.getItem('GodCheck') != undefined && localStorage.getItem('GodCheck') == 'YES'){
          alert('CANT REMOVE THE PERSON AS HE/SHE IS THE GOD OR THE CREATER OF THE GROUP')
          localStorage.removeItem('Checkmake');
          localStorage.removeItem('makeadminemail');
          localStorage.removeItem('GodCheck')
          document.getElementById('addmem').removeAttribute('readonly')
        ...
        firebase.database().ref('REMOVEM/' + localStorage.getItem('NAMEOg') + '/').child(document.getElementById('addmem').value).update({
        R:'YES',
        });

        firebase.database().ref('GROUPCHECK/' + localStorage.getItem('NAMEOg') + '/').child(localStorage.getItem('makeadminemail')).remove();
        
        alert('THE PERSON IS REMOVES SUCCESFULLY !!!!!!!!!!!!!!!!!')
        window.location = 'index.html'
        }
      }else{
        alert('THE USER IS NOT IN YOUR GROUP BUT EXSISTS IN MWORLD.')
